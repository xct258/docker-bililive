...
find_local_keyframes() {
    local target=$1
    local range=10  # ±5 秒范围
    local start=$(echo "$target - $range/2" | bc -l)
    local end=$(echo "$target + $range/2" | bc -l)
    (( $(echo "$start < 0" | bc -l) )) && start=0
    (( $(echo "$end > $video_duration_sec" | bc -l) )) && end=$video_duration_sec
    ...
    local prev=0
    local next=$video_duration_sec

    for f in "${frames[@]}"; do
        if (( $(echo "$f <= $target" | bc -l) )); then
            prev=$f
        else
            next=$f
            break
...
for seg in "${sorted_ranges[@]}"; do
    ...
    # 对齐关键帧
    read aligned_start _ <<< "$(find_local_keyframes "$start_sec")"
    read _ aligned_end <<< "$(find_local_keyframes "$end_sec")"

    ### 模式 1 & 3：保留未删除部分 ###
    if [[ "$operation" == "1" || "$operation" == "3" ]]; then
        if (( prev_end_sec < aligned_start )); then
            part_file="./keep_${index_keep}.mp4"
            echo "file '$part_file'" >> "$concat_keep"

            ffmpeg -ss "$(sec_to_hms "$prev_end_sec")" \
                   -to "$(sec_to_hms "$aligned_start")" \
                   -i "$input_video" \
                   -c copy -y "$part_file"
    ...
    if [[ "$operation" == "2" ]]; then
        ...
        ffmpeg -ss "$(sec_to_hms "$aligned_start")" \
               -to "$(sec_to_hms "$aligned_end")" \
               -i "$input_video" \
               -c copy -y "$part_file"
